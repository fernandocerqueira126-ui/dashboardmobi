...
export function ChatPanel({
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div key={group.date}>
                ...
                {group.messages.map((msg, idx) => {
                  ...
                  return (
                    <div
                      key={msg.id}
                      className={`flex mb-1 ${isAtendente ? "justify-start" : "justify-end"}`}
                    >
                      <div
                        className={`relative max-w-[65%] rounded-lg px-3 py-1.5 shadow-sm ${
                          isAtendente
                            ? "bg-card text-foreground border border-border/40 rounded-tr-[4px]"
                            : "bg-primary/90 text-primary-foreground rounded-tl-[4px]"
                        }`}
                      >
                        {idx === 0 && (
                          <div
                        ...
                        {!isAtendente && (
                          <p className="text-[11px] font-medium text-primary mb-0.5 mt-0.5">
                            {atendimento.clienteNome}
                          </p>
                        )}

                        {msg.imageUrl ? (
                          <div className="flex flex-col gap-1.5 mb-2 mt-1 min-w-[150px] max-w-[280px]">
                            <img 
                              src={msg.imageUrl} 
                              alt="Anexo" 
                              className="rounded-md object-cover max-h-[250px] w-full border border-border/10" 
                              loading="lazy" 
                            />
                            {msg.texto && <p className="text-[13px] leading-relaxed pr-10">{msg.texto}</p>}
                          </div>
                        ) : (
      ...
      {suggestion && (
        <div className="absolute bottom-[60px] left-4 right-4 z-10 animate-in slide-in-from-bottom-2 fade-in duration-300">
           <div className="bg-primary/10 border border-primary/20 backdrop-blur-md p-3 rounded-lg shadow-lg flex flex-col gap-2">
             <div className="flex justify-between items-center">
               <span className="text-xs font-semibold text-primary flex items-center gap-1"><CheckCircle className="w-3 h-3"/> Sugestão da IA (Antigravity):</span>
               <button onClick={onClearSuggestion} className="text-muted-foreground hover:text-foreground"><Trash2 className="w-3.5 h-3.5"/></button>
             </div>
             <p className="text-sm italic text-foreground/90">&ldquo;{suggestion}&rdquo;</p>
             <Button size="sm" className="w-full mt-1 bg-primary text-primary-foreground text-xs h-7" onClick={() => {
                setTexto(suggestion);
      ...
      <div className="px-3 py-2 bg-secondary/40 border-t border-border flex items-center gap-2 relative z-20">
